...
class BookDaoJpaTest {
    ...
    public void shouldAddNewBook() {
        ...
        long id = bookDaoJpa.insertOrUpdateBook(expectedBook);
        assertThat(id).isEqualTo(EXPECTED_BOOK_ID);

        Book actualBook = em.find(Book.class, EXPECTED_BOOK_ID);
        String actualName = actualBook.getAuthor().getName();
        String actualGenre = actualBook.getGenre().getGenre();
    ...
    public void shouldGetBookById() {
        Book book = bookDaoJpa.getBookById(ACTUAL_BOOK_ID).get();
    ...
    @Test
    @DisplayName("должен удалять книгу по id и комментарии к ней")
    public void shouldRemoveBookById() {
        bookDaoJpa.deleteBookById(ACTUAL_BOOK_ID);
        Optional<Book> book = bookDaoJpa.getBookById(ACTUAL_BOOK_ID);
